...
function App() {
    ...
    const fetchMtrData = async () => {
      ...
      const promises = mtrLines.map(async (line) => {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `HTTP error! status: ${response.status} for line ${line.code}`,
            );
          }
          ...
          console.error(
            `Failed to fetch or process data for ${line.name}:`,
            error,
          );
  ...
  return (
    <div style={{ position: "relative", width: "100vw", height: "100vh" }}>
      ...
      <img src={wheelsIcon} alt="Wheels Icon" className="wheels-icon" />
    </div>
  );
}
